...
models <- list(
  list(name = "Model 1: int + int + lineafm$", res = res1, color = "blue"),
  ...
  list(name = "Model 11: BKT", res = res10, color = "gold3"))
...
# Update model names with new rankings
for (i in seq_along(models)) {
  original_description <- sub("^Model \\d+: ", "", models[[i]]$name)
  models[[i]]$name <- paste0("Model ", i, ": ", original_description)
}
...
# RMSE Plot
png(rmse_filename, width = 6.5 * 300, height = 3.5 * 300, res = 300)
par(mar = c(5, 4, 2, 2))
plot(models[[1]]$res$RMSE, type = "o", pch = 16, col = models[[1]]$color, 
     ylim = rmse_ylim, xlab = "Percentage of Data Used in Training (%)", ylab = "RMSE")
for (i in 2:length(models)) {
  lines(models[[i]]$res$RMSE, type = "o", pch = 16, col = models[[i]]$color)
}
legend("topright", legend = sapply(models, function(m) m$name), 
       col = sapply(models, function(m) m$color), pch = 16, lty = 1, cex = 0.5)
dev.off()

# AUC Plot
png(auc_filename, width = 6.5 * 300, height = 3.5 * 300, res = 300)
par(mar = c(4, 3, 1, 1))
plot(models[[1]]$res$AUC, type = "o", pch = 16, col = models[[1]]$color,
     ylim = auc_ylim, xlab = "Percentage of Data Used in Training (%)", ylab = "AUC")
for (i in 2:length(models)) {
